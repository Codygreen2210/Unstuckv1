...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-cream-200 bg-cream-50 p-4">
      ...
      <div className="font-display text-3xl md:text-4xl mt-1.5 leading-none">{value}</div>
    </div>
  );
}
...
function QuickCard({
  ...
  return (
    <Link
      href={href}
      className="block rounded-2xl border border-cream-200 bg-cream-50 p-5 hover:border-rust-400/60 hover:bg-cream-100/60 transition-colors"
    >
      <Icon className="h-5 w-5 text-rust-600" />
      <div className="font-display text-lg mt-3">{title}</div>
      <div className="text-sm text-ink-muted mt-1">{body}</div>
    </Link>
  );
}
